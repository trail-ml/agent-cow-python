...
export async function deployCowFunctions(): Promise<void> {
  await exec(SETUP_COW_SQL);
  await exec(COMMIT_COW_OPERATIONS_SQL);
  await exec(DISCARD_COW_OPERATIONS_SQL);
  await exec(GET_COW_DEPENDENCIES_SQL);
}

export async function setupCowForTable(viewName: string, pkCol: string): Promise<void> {
  const baseTable = `${viewName}_base`;
  await query(`SELECT setup_cow($1, $2, $3)`, [baseTable, viewName, pkCol]);
}

export async function commitOperations(viewName: string, pkCol: string, sessionId: string, operationIds: string[]): Promise<void> {
  const baseTable = `${viewName}_base`;
  await query(`SELECT commit_cow_operations($1, $2, $3, $4)`, [baseTable, pkCol, sessionId, operationIds]);
}

export async function discardOperations(viewName: string, sessionId: string, operationIds: string[]): Promise<void> {
  const baseTable = `${viewName}_base`;
  await query(`SELECT discard_cow_operations($1, $2, $3)`, [baseTable, sessionId, operationIds]);
}

export async function getDependencies(sessionId: string): Promise<{ depends_on: string; operation_id: string }[]> {
  return query(`SELECT depends_on, operation_id FROM get_cow_dependencies($1)`, [sessionId]);
}
